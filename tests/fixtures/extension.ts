...
// so tests invoke functions through `serviceWorker.evaluate(...)`.
...
// adds ~1s of launch overhead and a visible window flash.
//
// `getServiceWorker` is *test-scoped* (and depends on the worker-scoped
...
type WorkerFixtures = {
  // Local HTTP server fixture: serves fixture HTML pages out of
  // tests/fixtures/pages/. We need a real http:// origin (rather than
  // file:// or data:) because:
  ...
  //     permission, so captures aren't authorized.
  //
  // Worker-scoped because it shares its lifetime with `extensionContext`
  // — one server per Playwright worker, reused across all tests.
...
export const test = base.extend<TestFixtures, WorkerFixtures>({
  fixtureServer: [
    async ({}, use) => {
      ...
      // server.close() doesn't block on them.
  ...
  getServiceWorker: async ({ extensionContext }, use) => {
    ...
    // underlying CDP target has already been torn down. So we don't
    // cache — every call walks the current list and verifies each
    // candidate with a no-op evaluate, falling back to waitForEvent
    // for a fresh spawn if nothing in the list is live.
