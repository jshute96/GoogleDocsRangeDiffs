...
import {
  ...
  switchDropdown,
  captureDiffRangeLogs,
  lastRewroteRange,
  reloadExtension,
} from './helpers';

// Reload the extension once per worker so it picks up a freshly built dist/.
// We can't use `beforeAll` because our `context` fixture is test-scoped;
// instead use a module-level once-guard in beforeEach.
...
test('initial entry: first item selected with From/To highlighted', async ({
  context,
  testDocUrl,
}) => {
...
test('click an unselected version: selects it, From+To land on it', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('click the date/label (opens rename textarea) also captures range', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('From on older item: range spans From..existing-To', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('To on newer item after a valid From: moves only the To highlight', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('To on an item OLDER than current From: captures both bounds on click target', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('with a range selected, clicking an item (inside or outside) resets to from=to', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('dropdown switch (to Named, then back to All) gives a clean selected range', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('reselecting the current dropdown option still resets and re-inits', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('exit and re-enter Version History: first item selected, From/To re-armed', async ({
  context,
  testDocUrl,
}) => {
  const page = await openDocAndVersionHistory(context, testDocUrl);
...
test('URL rewrite: setting From=item[2], To=item[0] sends start=item2.start, end=item0.end', async ({
  context,
  testDocUrl,
}) => {
  const page = await context.newPage();
