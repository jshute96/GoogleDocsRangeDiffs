...
- **The `Highlight changes` checkbox is the steering wheel.** We treat our `Diffs|Versions` toggle as the user-facing control and manipulate Highlight changes ourselves to get the kind of read we need. The checkbox is hidden from the user (see "Hidden Highlight-changes checkbox" above) so polarity-fix toggles happen invisibly.
